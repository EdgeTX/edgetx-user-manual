...
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/2n2y0XsJcrhXt3asceP0/color-radios/model-settings/telemetry/sensor-configuration-options
---
...
* **Formula:** Type of calculation to use. Options include:
  ...
  * **Cell:** This is the formula for FrSKY Lipo battery sensor. It uses the **Cell sensor** field to select the source sensor, and displays the cell voltage specified by the number in "Cell index" field.\
  ...
  * **Consumpt:** Calculates the power consumption (mAh) by cumulatively adding the values of the sensor selected in the **Current sensor** field.
  * **Distance:** Calculates the distance between the receiver and the radio using the sensors selected in the **GPS sensor** and **Alt sensor** fields.
* **Source 1, 2, 3, 4:** For the Add, Average, Minimum, Maximum, Multiply, and Totalize formulas, these are the sensors that will provide the argument values used in the formula defined above. (The Cell, Consumpt, and Distance formulas instead use their own dedicated source fields, described above.)
